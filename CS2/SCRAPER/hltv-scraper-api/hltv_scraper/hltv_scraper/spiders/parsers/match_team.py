...
class MatchTeamParser(Parser):
    ...
    def parse(teams_box, number: int) -> dict[str, Any]:
        link = teams_box.css(f"div.team{number}-gradient a::attr(href), a.dropdownTeam.team{number}::attr(href)").get()
        return {
            ...
            "score": teams_box.css(f".team{number}-gradient > div:nth-child(2)::text").get(),
        }
