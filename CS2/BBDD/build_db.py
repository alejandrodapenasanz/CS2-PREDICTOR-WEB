...
def backfill_prediction_opportunities(conn: sqlite3.Connection) -> int:
    """Populate opportunity metadata for predictions created before schema v1."""
    conn.execute(
        """
        UPDATE predictions
        SET opportunity_score = decision_confidence * reliability_score,
            opportunity_eligible = CASE
                WHEN decision_confidence >= ? AND reliability_score >= ? THEN 1
                ELSE 0
            END,
    ...
    rows = conn.execute(
        """
        SELECT p.prediction_id, p.model_version, p.opportunity_score,
               p.decision_confidence, substr(m.datetime_utc, 1, 10) AS match_day
        FROM predictions p
        JOIN matches m ON m.hltv_match_id = p.hltv_match_id
        WHERE p.opportunity_eligible = 1
        """
    ).fetchall()
    by_version: dict[str, list[tuple[Any, ...]]] = defaultdict(list)
    by_version_day: dict[tuple[str, str], list[tuple[Any, ...]]] = defaultdict(list)
    for row in rows:
    ...
    for version_rows in by_version.values():
        for rank, row in enumerate(sorted(version_rows, key=sort_key), start=1):
            updates[int(row[0])][0] = rank
    for day_rows in by_version_day.values():
        for rank, row in enumerate(sorted(day_rows, key=sort_key), start=1):
            updates[int(row[0])][1] = rank
            updates[int(row[0])][2] = rank == 1
...
def ensure_live_schema(conn: sqlite3.Connection) -> None:
    ...
    if opportunity_schema_changed:
        backfill_prediction_opportunities(conn)
