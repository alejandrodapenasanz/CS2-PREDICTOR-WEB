...
def _parameter_payload(
    *,
    ...
    extras = (script_path,) if script_path is not None and script_path.is_file() else ()
    return {
        "artifact_version": MODEL_ARTIFACT_VERSION,
        "feature_source": {
            "manifest_path": feature_source.path.relative_to(
                STATE_ROOT if feature_source.path.is_relative_to(STATE_ROOT) else PROJECT_ROOT
            ).as_posix(),
            ...
            "historical_odds_available": (feature_source.historical_odds_available),
...
def _safe_rmtree_staging(staging: Path, output_dir: Path) -> None:
    ...
    if resolved.exists() and resolved.parent == output and resolved.name.startswith(".staging-"):
...
def _serialize_gender_models(
    ...
    *,
    ...
    paths = {
        ...
        "ranking_probability": (gender_dir / "ranking_probability.joblib"),
        "deployment_bundle": gender_dir / "deployment_bundle.joblib",
    }
    dump_joblib_artifact(models.lightgbm, paths["lightgbm"])
    dump_joblib_artifact(models.lightgbm_calibrator, paths["lightgbm_calibrator"])
    dump_joblib_artifact(models.logistic, paths["logistic"])
    dump_joblib_artifact(models.logistic_calibrator, paths["logistic_calibrator"])
    dump_joblib_artifact(models.ranking_probability, paths["ranking_probability"])
    deployment_bundle = {
        "artifact_version": MODEL_ARTIFACT_VERSION,
        ...
        "training_excluded_unavailable": (models.training_excluded_unavailable),
        "training_max_date": models.training_max_date.isoformat(),
        "training_available_max_date": (models.training_available_max_date.isoformat()),
    ...
    dump_joblib_artifact(deployment_bundle, paths["deployment_bundle"])
    return {
        "gender": models.gender,
        "training_rows": models.training_rows,
        "training_source_rows": models.training_source_rows,
        ...
        "paths": {key: path.relative_to(staging).as_posix() for key, path in paths.items()},
    }
...
def _gender_summary(
    *,
    ...
    return {
        ...
        "training_available_max_date": (models.training_available_max_date.isoformat()),
        "training_as_of_date": models.training_as_of_date.isoformat(),
        "evaluation_rows": int(evaluation.market_audit.total_rows),
        "folds": fold_count,
...
def sync_active_documentation(published: PublishedRun) -> None:
    ...
    if not report_source.is_file():
        raise TrainingError(f"El run no contiene su informe: {report_source}.")
    _copy_atomic(report_source, DOCS_DIR / "model_report.md")
    for gender in ("M", "F"):
        plot_source = run_dir / "evaluation" / f"calibration_{gender}.png"
        if not plot_source.is_file():
            raise TrainingError(f"El run no contiene la curva {plot_source}.")
        _copy_atomic(
            plot_source,
...
def _load_run_tables(
    ...
    if not isinstance(summaries, list) or not all(isinstance(item, Mapping) for item in summaries):
...
def retrain_models(
    *,
    ...
    lightgbm_parameters: LightGBMParameters = (DEFAULT_LIGHTGBM_PARAMETERS),
    evaluation_parameters: TemporalEvaluationParameters = (DEFAULT_TEMPORAL_EVALUATION_PARAMETERS),
    ...
    resolved_training_cutoff = (
        datetime.now(UTC).date() if training_as_of_date is None else training_as_of_date
    )
    if isinstance(resolved_training_cutoff, datetime) or not isinstance(
        resolved_training_cutoff, date
    ):
    ...
    metadata = tuple(verify_training_dataset(feature_source, gender) for gender in ("M", "F"))
    ...
    canonical_output = Path(output_dir).resolve() == PHASE7_MODELS_DIR.resolve()
    ...
    try:
        for dataset_metadata in metadata:
            ...
            causal_mask = available_dates.map(lambda value: value < resolved_training_cutoff)
            causal_frame = loaded.frame.loc[causal_mask].copy()
            if causal_frame.empty:
                raise TrainingError(f"{gender}: el corte no deja resultados disponibles.")
            if not (
                pd.to_datetime(causal_frame["result_available_date"], errors="raise").dt.date
                < resolved_training_cutoff
            ).all():
                raise TrainingError(f"{gender}: el subset causal no reconcilia con el corte.")
            ...
            evaluation = evaluate_gender_backtest(backtest, parameters=evaluation_parameters)
            ...
            save_reliability_plot(
                ...
                gender=gender,
                output_path=(evaluation_dir / f"calibration_{gender}.png"),
            )
        ...
        nonempty_suspicious = [frame for frame in all_suspicious if not frame.empty]
        suspicious = (
            pd.concat(nonempty_suspicious, ignore_index=True)
            if nonempty_suspicious
            else pd.DataFrame(columns=all_suspicious[0].columns if all_suspicious else ())
        )
        market = pd.DataFrame(market_rows)
        folds = pd.DataFrame(fold_rows)
        evaluation_dir = staging / "evaluation"
        _write_dataframe_csv(metrics, evaluation_dir / "metrics.csv")
        _write_dataframe_csv(reliability, evaluation_dir / "reliability_bins.csv")
        _write_dataframe_csv(suspicious, evaluation_dir / "suspicious_segments.csv")
        _write_dataframe_csv(market, evaluation_dir / "market_coverage.csv")
        _write_dataframe_csv(folds, evaluation_dir / "folds.csv")

        run_relative_from_docs = (
            Path("..") / "models" / "phase7" / "runs" / fingerprint
        ).as_posix()
        report = render_model_report(
            ...
            source_commit=feature_source.source_commit,
            profile=resolved_profile,
            evaluation_first_year=(evaluation_parameters.first_test_season),
            evaluation_last_year=(evaluation_parameters.last_test_season),
        ...
        write_model_report(report, staging / "report" / "model_report.md")

        global_metrics = metrics.loc[
            (metrics["population"] == "native") & (metrics["scope"] == "global")
        ]
