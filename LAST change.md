...
1. **Datos presentes** (no están en git; deben existir en este PC):
   - `SCRAPPER/hltv-scraper-api/hltv_scraper/data/raw/history_10000_*/results_all.json`
   - `DAILY_SNAPSHOTS/master/matches.json` (opcional; extiende el histórico hasta hoy)
...
bloqueado. Revisa `DAILY_SNAPSHOTS/runs/<run>/fetch_diagnostics.json`: busca
...
`DAILY_SNAPSHOTS/start.py` (se reutilizan todas las guardas previas: presupuesto,
...
- **CA corporativa (redes con inspección TLS)**: `start.ps1::Ensure-CaBundle` hace un
  ...
  `SCRAPPER/hltv-scraper-api/corp_ca_bundle.pem` y lo publica por env. **En un PC sin
...
Archivos tocados (scraper): `DAILY_SNAPSHOTS/start.py`, `start.ps1`,
`SCRAPPER/hltv-scraper-api/requirements.txt` (`+ scrapling[fetchers]`).
...
- **`corp_ca_bundle.pem`** y `SCRAPPER/Scrapling/` (clon local) están gitignored:
