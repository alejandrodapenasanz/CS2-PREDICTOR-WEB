...
DAILY_SNAPSHOTS/runs/<run_id>/ y hace upserts idempotentes en BBDD/cs2.db.
...
def insert_predictions(conn: sqlite3.Connection, run_dir: Path) -> int:
    ...
    for row in payload:
        ...
        current_ids.add(hltv_id)
        match_id = build_db._match_id_for_hltv(conn, hltv_id)
        conn.execute(
            """
            INSERT INTO predictions(
                match_id, hltv_match_id, model_version, predicted_at_utc, prob_team1,
                odds_prob_team1, blended_prob_team1, risk_adjusted_prob_team1,
                decision_prob_team1, decision_confidence, decision_probability_source,
                decision_market_weight, reliability_score, confidence,
                prediction_json, features_json, odds_json, staking_json, controls_json,
                flags_json, data_quality_json, rosters_json
            )
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
            ON CONFLICT(hltv_match_id, model_version) DO UPDATE SET
                ...
                odds_prob_team1=excluded.odds_prob_team1,
                blended_prob_team1=excluded.blended_prob_team1,
                risk_adjusted_prob_team1=excluded.risk_adjusted_prob_team1,
                decision_prob_team1=excluded.decision_prob_team1,
                prediction_json=excluded.prediction_json,
                features_json=excluded.features_json,
                odds_json=excluded.odds_json,
                staking_json=excluded.staking_json,
                controls_json=excluded.controls_json,
                flags_json=excluded.flags_json,
                data_quality_json=excluded.data_quality_json,
                rosters_json=excluded.rosters_json
            """,
            (
                match_id,
                hltv_id,
                version,
                row.get("captured_at") or utcnow(),
                prob,
                ...
                json.dumps(row.get("odds"), ensure_ascii=False),
                json.dumps(row.get("staking"), ensure_ascii=False),
                json.dumps(row.get("controls"), ensure_ascii=False),
                json.dumps(row.get("flags"), ensure_ascii=False),
                json.dumps(row.get("data_quality"), ensure_ascii=False),
                json.dumps(row.get("rosters"), ensure_ascii=False),
            ),
        )
        inserted += 1
    if current_ids:
        ...
        conn.execute(
            f"DELETE FROM predictions WHERE model_version=? AND hltv_match_id NOT IN ({placeholders})",
            (version, *sorted(current_ids)),
        )
    return inserted
...
def latest_run_dir() -> Path:
    manifest = read_json(ROOT / "DAILY_SNAPSHOTS" / "master" / "manifest.json", {})
    ...
    if not runs:
        raise FileNotFoundError("No hay runs en DAILY_SNAPSHOTS/runs")
    return runs[-1]
